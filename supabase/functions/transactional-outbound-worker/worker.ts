import type {
  EmailProvider,
  ProviderReceipt,
  SmsProvider,
} from '../../../src/core/integrations/contracts.ts';
import { IntegrationError } from '../../../src/core/integrations/contracts.ts';
import type { TransactionalClaim, TransactionalWorkerResult } from './contracts.ts';

export type TransactionalProviders = {
  sms: SmsProvider;
  email: EmailProvider;
};

export interface TransactionalWorkerRepository {
  authorizeLiveSend?(claim: Extract<TransactionalClaim, { operation: 'send' }>): Promise<void>;
  beginSubmission(attemptId: string, claimToken: string, providerName: string): Promise<void>;
  complete(
    attemptId: string,
    claimToken: string,
    receipt: ProviderReceipt,
  ): Promise<TransactionalWorkerResult>;
  fail(
    attemptId: string,
    claimToken: string,
    errorCode: string,
    retryable: boolean,
  ): Promise<TransactionalWorkerResult>;
  markSubmissionUnknown(
    attemptId: string,
    claimToken: string,
    errorCode: string,
    providerName: string,
    providerMessageId?: string,
  ): Promise<TransactionalWorkerResult>;
}

function providerFor(claim: TransactionalClaim, providers: TransactionalProviders) {
  return claim.channel === 'sms' ? providers.sms : providers.email;
}

function safeErrorCode(error: unknown): {
  code: string;
  retryable: boolean;
} {
  if (error instanceof IntegrationError) {
    const code = error.code
      .toUpperCase()
      .replace(/[^A-Z0-9_.:-]/gu, '_')
      .slice(0, 120);
    return {
      code: code || 'PROVIDER_ERROR',
      retryable: error.retryable,
    };
  }
  return { code: 'WORKER_INTERNAL_ERROR', retryable: true };
}

const SAFE_BEFORE_PROVIDER_SUBMISSION_ERRORS = new Set([
  'CONSENT_NOT_GRANTED',
  'STALE_CONSENT',
  'OPTED_OUT',
  'CONSENT_CONTACT_MISMATCH',
  'CONSENT_UNKNOWN',
  'CONSENT_UNAVAILABLE',
  'RATE_LIMITED',
  'STORYOPS_COMPANY_NOT_ACTIVE',
  'TRANSACTIONAL_ENTITY_CHANGED',
  'SUBMISSION_BOUNDARY_UNAVAILABLE',
]);

function failedBeforeProviderSubmission(error: unknown): boolean {
  return (
    error instanceof IntegrationError && SAFE_BEFORE_PROVIDER_SUBMISSION_ERRORS.has(error.code)
  );
}

function send(
  claim: Extract<TransactionalClaim, { operation: 'send' }>,
  providers: TransactionalProviders,
): Promise<ProviderReceipt> {
  if (claim.channel === 'sms') {
    return providers.sms.sendSms({
      companyId: claim.companyId,
      to: claim.recipient,
      body: claim.body,
      category: 'transactional',
      consentSnapshotId: claim.consentSnapshotId,
      idempotencyKey: claim.idempotencyKey,
    });
  }
  return providers.email.sendEmail({
    companyId: claim.companyId,
    to: [claim.recipient],
    subject: claim.subject,
    text: claim.body,
    category: 'transactional',
    consentSnapshotIds: {
      [claim.recipient]: claim.consentSnapshotId,
    },
    idempotencyKey: claim.idempotencyKey,
  });
}

async function reconcile(
  claim: Extract<TransactionalClaim, { operation: 'reconcile' }>,
  providers: TransactionalProviders,
): Promise<ProviderReceipt> {
  const provider = providerFor(claim, providers);
  if (provider.mode !== 'live') {
    throw new IntegrationError(
      'A live provider is required to reconcile a live submission.',
      provider.provider,
      'LIVE_PROVIDER_REQUIRED_FOR_RECONCILIATION',
      true,
    );
  }
  if (provider.provider !== claim.providerName) {
    throw new IntegrationError(
      'The configured provider does not match the durable submission.',
      provider.provider,
      'PROVIDER_CONFIGURATION_CHANGED',
      true,
    );
  }
  const receipt =
    claim.channel === 'sms'
      ? await providers.sms.getSmsDelivery(claim.providerMessageId)
      : await providers.email.getEmailDelivery(claim.providerMessageId);
  if (!receipt) {
    throw new IntegrationError(
      'The provider has not returned a delivery receipt yet.',
      provider.provider,
      'PROVIDER_RECEIPT_NOT_FOUND',
      true,
    );
  }
  return { ...receipt, idempotencyKey: claim.idempotencyKey };
}

export async function processTransactionalClaim(options: {
  claim: TransactionalClaim;
  providers: TransactionalProviders;
  repository: TransactionalWorkerRepository;
}): Promise<TransactionalWorkerResult> {
  const provider = providerFor(options.claim, options.providers);
  const sendClaim = options.claim.operation === 'send' ? options.claim : undefined;
  const liveSubmission = sendClaim !== undefined && provider.mode === 'live';
  let submissionBoundaryPersisted = false;
  let receipt: ProviderReceipt | undefined;
  try {
    if (liveSubmission) {
      await options.repository.authorizeLiveSend?.(sendClaim);
      await options.repository.beginSubmission(
        options.claim.attemptId,
        options.claim.claimToken,
        provider.provider,
      );
      submissionBoundaryPersisted = true;
    }
    receipt =
      options.claim.operation === 'send'
        ? await send(options.claim, options.providers)
        : await reconcile(options.claim, options.providers);
    if (receipt.idempotencyKey !== options.claim.idempotencyKey) {
      throw new IntegrationError(
        'The provider receipt did not preserve the durable idempotency key.',
        receipt.provider,
        'PROVIDER_RECEIPT_IDEMPOTENCY_MISMATCH',
        false,
      );
    }
    return await options.repository.complete(
      options.claim.attemptId,
      options.claim.claimToken,
      receipt,
    );
  } catch (error) {
    const failure = safeErrorCode(error);
    if (submissionBoundaryPersisted && !failedBeforeProviderSubmission(error)) {
      return options.repository.markSubmissionUnknown(
        options.claim.attemptId,
        options.claim.claimToken,
        failure.code,
        provider.provider,
        receipt?.providerId,
      );
    }
    return options.repository.fail(
      options.claim.attemptId,
      options.claim.claimToken,
      failure.code,
      failure.retryable,
    );
  }
}
